...
캔버스 툴바의 **AI에게 묻기**를 누르면 지금 화면의 브리프(구성 · 선택 옵션 · 제품과 정격 ·
동작점 · 전력 수지 · 선택한 지점의 신호와 수식 · 룰 판정 · 아직 확인되지 않은 것)를 근거로
질문할 수 있다. 브리프는 `src/analysis/brief.ts`가 만드는 순수 함수의 출력이고,
**화면에 있는 것만 들어간다** — 여기서 새로운 사실을 만들지 않는다.

배포에 따라 동작이 다르다.

| 배포 | 동작 |
|---|---|
| Artifact (claude.ai) | 화면 안에서 바로 묻는다. **보는 사람의 Claude 계정**으로 처리되며 API 키가 필요 없다. 첫 질문에서 허용을 한 번 묻는다 |
| GitHub Pages · 로컬 | 물을 통로가 없다. 브리프 복사 버튼으로 Claude에 붙여넣는다 |

답에는 "AI가 브리프를 근거로 쓴 글이며 검증된 공학 판단이 아니다"라는 표기가 함께 붙는다.
질문에 실려 가는 지시문(`ASK_INSTRUCTION`)이 이 프로젝트의 규율을 그대로 건다 —
제품 스펙을 지어내지 말 것, 조항을 기억으로 인용하지 말 것, 모르면 모른다고 할 것.

`npm run artifact`는 빌드 결과를 파일 하나(`dist-artifact/index.html`)로 접는다.
비ASCII 문자는 `\uXXXX`로 이스케이프한다 — 문자셋 선언이 없는 곳에 인라인되면
한글 정규식이 깨지기 때문이다(로컬 검증에서 확인).
