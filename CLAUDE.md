...
- 발전량 시뮬레이션 없음 (필요해지면 pvlib 결과를 CSV로 주입).
  **정상상태 동작점**(일사·부하를 주면 각 지점의 V·I·P가 얼마인가)은 예외다 —
  임피던스·전압 강하·무효전력을 풀지 않고, 기상 데이터도 SOC 궤적도 없다.
  하루 시간축(위도·월·시각 → 맑은 하늘 일사 근사)까지는 다루되, **시간 적분은 하지 않는다** —
  매 시각이 각각 독립적인 한 점이고, 그 점들을 늘어놓은 것이 하루 곡선이다.
  누적 발전량(kWh)을 내놓기 시작하면 그때부터는 시뮬레이터이므로 하지 않는다
...
locations/          사이트 위치 (YAML, 1지역 1파일) — 일사 계산의 기하 입력
...
AC 모듈(모듈 + 공장 결합 마이크로인버터)은 별도 class가 아니다. 모듈 노드와 인버터 노드로
나눠 그린다 — 그래야 모듈의 DC 출력을 볼 수 있다. 실물이 한 함체이고 현장에 DC 배선이
노출되지 않는다는 사실은 device의 `sources`/`todos`에 적는다.
...
| Qcells Q.HOME (Q.TRON AC + COMBINER + HUB G3 + CORE G3) | **AC 결합**. 모듈 + 내장 마이크로인버터가 트렁크로 결합반에 모인다 |
...
일사는 두 가지 방법으로 정해진다. 지역(위도)·월·시각을 고르면 **맑은 하늘 근사**로
경사면 일사를 계산하고(`src/analysis/solar.ts`), 지역을 고르지 않으면 일사비를 손으로 준다.
근사 모델은 구름·음영·온도·오염을 반영하지 않으며, 그 사실을 화면에 함께 띄운다.
시각은 시나리오와 다른 축이다 — **시나리오는 회로의 상태(활선/사선), 시각은 크기**다.
...
│ 지역/월/부하   │  단자(점) 클릭 → 신호 패널     │  또는 노드 신호 │
│              │                              │  + 시간 슬라이더 │
└──────────────┴────────────────────────────┴──────────────┘
```
...
- 배열은 좌우로만 늘어놓지 않는다. 묶음(분기회로·스트링)마다 한 행을 쓰고, 짝지어진
  모듈·인버터는 세로로 포갠다. 한 화면에 들어오는 것이 읽히는 것의 전제다
- 신호를 보는 손잡이는 함체가 아니라 **단자(포트)**다. 도체가 제품을 떠나는 지점에
  점을 찍고, 그 점이 클릭 대상이다
- AI가 쓴 글은 그렇게 표시한다. 검증된 판단과 생성된 글을 같은 무게로 보여주지 않는다
...
- 백엔드 없음. GitHub Pages 정적 배포. Artifact 배포는 `npm run artifact`가 만드는 단일 HTML
- **AI 질문은 배포 환경의 기능이지 앱의 의존성이 아니다.** Artifact 뷰어 안에서는
  `claude.use("sample")`로 보는 사람의 Claude 계정을 통해 묻고, 그 통로가 없는 배포에서는
  브리프를 복사해 가게 한다. 없는 것이 정상이므로 앱은 그것 없이도 완전히 돈다
...
| 8 | 단자 선택 + 하루 시간축 + 격자 배치 | 단자를 눌러 신호를 보고, 재생하면 하루가 흐르고, 도면이 한 화면에 들어옴 |
| 9 | AI 질문 패널 + Artifact 배포 | 화면의 브리프를 근거로 묻고, 통로가 없으면 복사해 간다 |
